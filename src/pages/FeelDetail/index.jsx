...
import "../FeelDetail/style.css";
import { Link } from "react-router-dom";

const FeelDetail = () => {
  const [feel, setFeel] = useState([]);
  const user = useSelector((state) => state.auth.login?.currentUser);
  const { feelId } = useParams();
  const [editFeel, setEditFeel] = useState(false);
  useEffect(() => {
    axios
      .get(process.env.REACT_APP_URL_API + `/api/auth/feel/${feelId}`, {
        headers: {
  ...
  const sendLike = async (feel) => {
    ...
    const updatedFeel = { ...feel };

    // Cập nhật cảm xúc trên bản sao
    const liked = updatedFeel.likes.some((like) => like.user_id === user.id);
    if (liked) {
      updatedFeel.likes = updatedFeel.likes.filter(
        (like) => like.user_id !== user.id
      );
    } else {
      updatedFeel.likes.push({
        user_id: user.id,
        post_id: feel.id,
      });
    }

    // Cập nhật state của `feel` bằng bản sao đã được cập nhật
    setFeel(updatedFeel);
  };
  ...
  return (
    <div className="flex justify-center h-screen">
      <div className="mb-3 w-6/12 border-gray-300 bg-white shadow rounded-lg">
          ...
          <div className="flex flex-row">
            ...
            <div className="flex flex-col mb-2 ml-4 mt-1">
              <div className="text-gray-600 text-sm font-semibold">
                <Link to={`/user/${feel.user?.id}`}>{feel.user?.name}</Link>
              </div>
        ...
        <div className="text-gray-400 font-medium text-sm mb-7 mt-6 mx-3 px-2">
          {feel?.image && (
            <Image
              src={
                process.env.REACT_APP_URL_API + "/storage/images/" + feel?.image
              }
            />
          )}
        </div>
        ...
        >
          <div className="relative flex items-center self-center w-full max-w-xl p-4 overflow-hidden text-gray-600 focus-within:text-gray-400">
            <img
              ...
              src={
                process.env.REACT_APP_URL_API + "/storage/images/" + user.image
              }
              alt="avatar"
            />
